...
func (b *BackupSyncController) startBackupSync() {
	for {
		...
		case <-time.After(b.SyncInterval):
			backupLocations, err := stork.Instance().ListBackupLocations("")
			if err != nil {
	...
	for backupName := range backups {
		...
		for {
			...
			if object.IsDir {
				...
				localBackupInfo, err := stork.Instance().GetApplicationBackup(backupInfo.Name, backupInfo.Namespace)
				if err == nil {
				...
				_, err = stork.Instance().GetApplicationBackup(syncedBackupName, backupInfo.Namespace)
				...
				_, err = stork.Instance().CreateApplicationBackup(&backupInfo)
				if err != nil {
					return err
				}
