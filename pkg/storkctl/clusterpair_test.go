...
func createClusterPairAndVerify(t *testing.T, name string, namespace string) {
	...
	_, err := stork.Instance().CreateClusterPair(clusterPair)
	require.NoError(t, err, "Error creating Clusterpair")
	// Make sure it was created correctly
	clusterPair, err = stork.Instance().GetClusterPair(name, namespace)
...
func TestGetClusterPairsWithStatus(t *testing.T) {
	...
	clusterPair, err := stork.Instance().GetClusterPair("clusterpairstatustest", "default")
	require.NoError(t, err, "Error getting Clusterpair")
	...
	_, err = stork.Instance().UpdateClusterPair(clusterPair)
