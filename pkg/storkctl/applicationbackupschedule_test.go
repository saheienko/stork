...
func createApplicationBackupScheduleAndVerify(
	...
	_, err := stork.Instance().CreateSchedulePolicy(&storkv1.SchedulePolicy{
	...
	applicationbackup, err := stork.Instance().GetApplicationBackupSchedule(name, namespace)
...
func TestGetApplicationBackupSchedulesMultipleNamespaces(t *testing.T) {
	defer resetTest()
	_, err := stork.Instance().CreateNamespace("test1", nil)
	require.NoError(t, err, "Error creating test1 namespace")
	_, err = stork.Instance().CreateNamespace("test2", nil)
...
func TestGetApplicationBackupSchedulesWithStatus(t *testing.T) {
	...
	applicationBackupSchedule, err := stork.Instance().GetApplicationBackupSchedule("getapplicationbackupschedulestatustest", "default")
	require.NoError(t, err, "Error getting applicationbackup schedule")
	...
	applicationBackupSchedule, err = stork.Instance().UpdateApplicationBackupSchedule(applicationBackupSchedule)
...
func TestSuspendResumeApplicationBackupSchedule(t *testing.T) {
	...
	applicationBackupSchedule, err := stork.Instance().GetApplicationBackupSchedule(name, namespace)
