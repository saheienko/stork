...
func createApplicationRestoreAndVerify(
	...
	restore, err := core.Instance().GetApplicationRestore(name, namespace)
...
func TestGetApplicationRestoresWithStatusAndProgress(t *testing.T) {
	...
	restore, err := core.Instance().GetApplicationRestore("getrestorestatustest", "default")
	require.NoError(t, err, "Error getting restore")
	...
	_, err = core.Instance().UpdateApplicationRestore(restore)
